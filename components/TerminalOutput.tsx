...
const TerminalOutput: React.FC = () => {
  ...
  return (
    <div className="bg-[#0a0a0a] rounded-xl border border-white/10 overflow-hidden shadow-2xl w-full max-w-2xl">
      <div className="bg-[#121212] px-4 py-2 border-b border-white/10 flex items-center justify-between">
        ...
        <span className="text-[10px] uppercase tracking-widest text-gray-500 font-mono-tech">sh — x64 — production</span>
      </div>
      <div className="p-6 h-[240px] font-mono-tech text-sm leading-relaxed overflow-y-auto">
        {lines.slice(0, currentLineIndex).map((line, idx) => (
          <div key={idx} className={`${line.color} mb-1`}>{line.text}</div>
        ))}
        {currentLineIndex < lines.length && (
          <div className={`${lines[currentLineIndex].color} flex`}>
            {displayedText}
            <span className="w-2 h-5 bg-blue-500 ml-1 animate-pulse"></span>
          </div>
        )}
        {!isTyping && (
